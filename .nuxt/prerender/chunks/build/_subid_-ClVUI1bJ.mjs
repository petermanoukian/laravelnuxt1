...
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate, ssrRenderAttr, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderList } from 'file://C:/My-Documents/build4/node_modules/vue/server-renderer/index.mjs';
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const selectedSubcat = ref(subidx);
    useNuxtApp();
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (subcat.value) {
        _push(`<div><h1>${ssrInterpolate(subcat.value.name)}</h1><h2>${ssrInterpolate(subcat.value.cat.name)}</h2><p>ID: ${ssrInterpolate(unref(subidx))}</p>`);
        if (subcat.value.img) {
          _push(`<p><img${ssrRenderAttr("src", `${unref(config).public.BaseUrl}/images/subcat/thumb/${subcat.value.img}`)}${ssrRenderAttr("alt", subcat.value.name)} class="mt-2 mb-2"></p>`);
        } else {
          _push(`<p><img${ssrRenderAttr("src", _imports_0)} alt="Default Picture" class="mt-2 mb-2"></p>`);
        }
        _push(`</div>`);
      ...
      ssrRenderList(unref(subcats), (subcat2) => {
        _push(`<option${ssrRenderAttr("value", subcat2.id)}>${ssrInterpolate(subcat2.name)}</option>`);
      });
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/prod/viewprodbycatidsubid/[catid]/[subid].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_subid_-ClVUI1bJ.mjs.map
