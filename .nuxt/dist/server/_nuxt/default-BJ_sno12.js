...
import { ref, mergeProps, withCtx, createVNode, createTextVNode, toDisplayString, useSSRContext, watch, unref } from "vue";
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderList, ssrInterpolate, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderAttr, ssrRenderSlot } from "vue/server-renderer";
      ...
      ssrRenderList(cats.value, (cat) => {
        ...
        if (getSubcatsByCatId(cat.id).length && cat.subcats_count > 0 && cat.prods_count > 0) {
          ...
          _push(ssrRenderComponent(_component_NuxtLink, {
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`${ssrInterpolate(cat.name)} ( ${ssrInterpolate(cat.subcats_count)} ) | ( ${ssrInterpolate(cat.prods_count)} ) » `);
              } else {
                return [
                  createTextVNode(toDisplayString(cat.name) + " ( " + toDisplayString(cat.subcats_count) + " ) | ( " + toDisplayString(cat.prods_count) + " ) » ", 1)
                ];
              }
            }),
            _: 2
...
//# sourceMappingURL=default-BJ_sno12.js.map
