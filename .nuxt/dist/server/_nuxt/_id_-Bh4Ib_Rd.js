...
import { ref, onServerPrefetch, withAsyncContext, mergeProps, withCtx, createTextVNode, toDisplayString, unref, useSSRContext } from "vue";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (prod.value) {
        ...
        if (prod.value.des !== "null" && prod.value.des !== "NULL" && prod.value.des !== "") {
          _push(`<div class="formatted-result mt-4">${prod.value.des ?? ""}</div>`);
        } else {
          _push(`<!---->`);
        }
        if (prod.value.dess !== "null" && prod.value.dess !== "NULL" && prod.value.dess !== "") {
          _push(`<div class="formatted-result mt-4">${prod.value.dess ?? ""}</div>`);
        } else {
          _push(`<!---->`);
        }
        if (prod.value.img) {
...
//# sourceMappingURL=_id_-Bh4Ib_Rd.js.map
